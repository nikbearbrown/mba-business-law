...
*When a deal goes wrong, here is the apparatus that catches it.*
...
**Exercise 2.1.** Present this situation to an LLM: *A freelance software developer has completed 80 percent of a contracted project. The client refuses to pay the final milestone, claiming the deliverable doesn't meet specifications. The contract is silent on dispute resolution. The developer is owed $18,000.* Ask the LLM to recommend a dispute-resolution strategy using the funnel framework and the Thomas-Kilmann modes. Evaluate the LLM's output: Did it apply the modes correctly? Did it consider the cost of each level of the funnel relative to the amount in dispute? Did it recommend the right entry point?
